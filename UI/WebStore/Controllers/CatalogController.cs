    ...
    public class CatalogController : Controller
    {
        ...
        public IActionResult Index(int? BrandId, int? SectionId, int Page = 1, int? PageSize = null)
        {
            var page_size = PageSize
                ?? (int.TryParse(_Configuration[__PageSizeConfig], out var value)
                    ? value
                    : null);
        ...
        {
            var products = GerProducts(BrandId, SectionId, Page, PageSize);

            return PartialView("Partial/_Products", products);
        }

        private IEnumerable<ProductViewModel> GerProducts(int? BrandId, int? SectionId, int Page, int? PageSize = null)
        {
